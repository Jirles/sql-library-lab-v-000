...
def select_character_names_and_number_of_books_they_are_in
  "SELECT c.name, COUNT(cb.book_id) AS count FROM characters c INNER JOIN character_books cb ON c.id = cb.character_id GROUP BY c.id ORDER BY count, c.name DESC;"
end
