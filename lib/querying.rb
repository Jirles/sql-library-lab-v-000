...
def select_series_title_with_most_human_characters
  "SELECT s.title FROM series s INNER JOIN characters c ON c.series_id = s.id HAVING COUNT(SELECT species FROM characters WHERE species = 'human') AS count GROUP BY s.id ORDER BY count DESC LIMIT 1;"
end
